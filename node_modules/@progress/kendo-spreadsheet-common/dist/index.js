(function (global, factory) {
    ...
    class Context {
        ...
        fetchName(ref, callback) {
            ...
            if (val instanceof Formula$1) {
                ...
                var ss = new ValidationFormulaContext(this.ss.workbook);
    ...
    function compileArgumentChecks(functionName, args) {
        ...
        f = f(kendo, CalcError, limitPrecision);
        if (!hasArrayArgs) {
            delete f.arrayArgs;
        }
        return f;
    ...
    calc.runtime.packDate = packDate;
    calc.runtime.unpackDate = unpackDate;
    calc.runtime.packTime = packTime;
    calc.runtime.unpackTime = unpackTime;
    calc.runtime.serialToDate = serialToDate;
    calc.runtime.dateToSerial = dateToSerial;
    calc.runtime.daysInMonth = daysInMonth;
    calc.runtime.validDate = validDate;
    calc.runtime.isLeapYear = isLeapYear;
    ...
    defineFunction("_matrix", function(m) {
        if (typeof m == "string") {
    ...
    let makeClosure = (function(cache){
        return function(code) {
            let f = cache[code];
            if (!f) {
                f = cache[code] = new Function("kendo", "'use strict';return(" + code + ")")(kendo);
            }
            return f;
        };
    })(Object.create(null));
    ...
    function toCPS(ast, k) {
        ...
        function cpsLambda(node, k) {
            let cont = gensym("K");
            let body = cps(node.body, function(body){
                return { type: "call",
    ...
    function makeFormula(exp) {
        ...
        let code = js(toCPS(exp.ast, function(ret){
            return {
                type: "return",
                value: ret
            };
        ...
        code = [
            "function(){",
            "let context = this, refs = context.formula.absrefs",
            code,
            "}"
        ].join(";\n");

        formula = new calc.runtime.Formula(exp.refs, makeClosure(code), printer, exp.sheet, exp.row, exp.col);
    ...
    calc.InputStream = InputStream;
    calc.ParseError = ParseError;
    ...
    */
    ...
    function ACCRINTM(issue, maturity, rate, par, basis) { // ACCRINTM(issue, settlement, rate, par, [basis])
        var year_days = daysInYear2(unpackDate(maturity).year, basis); // 360;
    ...
    function PRICEDISC(settl, matur, discount, redemption, basis) { // PRICEDISC(settlement, maturity, discount, redemption, [basis])
        var dsm = _daysBetween(settl, matur, basis),
            dy = daysInYear2(unpackDate(matur).year, basis);
    ...
    function compileFormatPart(format) {
        ...
        let scaleCount = 0;
        let code = "let intPart, decPart, isNegative, date, time; ";
        ...
        if (condition === "text") {
            ...
            let op = condition.op === "=" ? "==" : condition.op;
        ...
        input.restart();
        code += "let matchedCurrency = false;";
        while (!input.eof()) {
            let tok = input.next();
            if (tok.type === "dec") {
                code += "output += culture.numbers.symbols.decimal; ";
            }
                ...
                code += "type = 'percent'; ";
                code += "output += culture.numbers.symbols.percentSign; ";
            }
            else if (tok.type === "str") {
    ...
    let compile = memoize(function(format) {
        ...
        let code = tree.map(compileFormatPart);
        code.push(TEXT);
        code = code.join("\n");
        code = "'use strict'; return function(value, culture){ "
            + "if (!culture) culture = kendo.culture();"
            + "let output = '', type = null, result = { body: [] }; " + code + "; return result; };";
        return new Function("kendo", "runtime", code)(kendo, runtime$1);
    });
    ...
    let Range$1 = class Range {
        ...
        input(value, options) {
            options = Object.assign({}, options, { arrayFormula: false });
    ...
    function numberToDate(val) {
        return val == null ? null : serialToDate(val);
    }
    ...
    class EditCommand extends PropertyChangeCommand {
        ...
        _exec() {
            ...
            try {
                ...
                if (ex instanceof calc.ParseError) {
                    return {
