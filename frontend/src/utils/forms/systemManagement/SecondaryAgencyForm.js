...
class SecondaryAgencyForm extends BaseForm {
  initialValue = (values) => ({
  ...
  validationSchema = () =>
    this.Yup.object().shape({
      ...
      enCode: this.Yup.string()
        .required('英文代碼必填！')
        .min(3, '英文代碼至少三個字！')
        .test('Invalid en code', '英文代碼必須為全大寫！', (enCode) =>
          FormikHelper.enCodeValidation(enCode),
        ),
    });
}

const secondaryAgencyForm = new SecondaryAgencyForm();

export default secondaryAgencyForm;
