...
class MobilizationPlanManagementActionCell extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  openEditPage = () => {
    ...
    props.history.push({
      pathname: Path.mobilizationClassificationManagementPath,
      search: query,
    });
  };
  ...
  deleteFunction = (callback) => {
    ...
    ApiService.mobilizationPlan
      .deleteMobilizationPlan(props.cell.row.original.id)
      .then((response) => {
...
MobilizationPlanManagementActionCell.propTypes = {
...
export default withRouter(
  connect(
    mapStateToProps,
    mapDispatchToProps,
  )(MobilizationPlanManagementActionCell),
);
