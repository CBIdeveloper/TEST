...
class SystemMenuActionCell extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  openEditPage = () => {
    ...
    props.history.push({
      pathname: Path.systemSubMenuPath,
      search: query,
    });
  };
  ...
  deleteFunction = (callback) => {
    const { props } = this;
    ApiService.menu
      .deleteMenu(props.cell.row.original.id)
      .then((response) => {
...
SystemMenuActionCell.propTypes = {
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(SystemMenuActionCell),
);
