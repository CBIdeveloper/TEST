...
class MobilizationExecutionActionCell extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  openEditPage = () => {
    ...
    props.history.push({
      pathname: UrlParser([
        ...
        Path.editMobilizationExecutionPath,
      ]),
      search: query,
    });
  };
  ...
  deleteFunction = (callback) => {
    ...
    ApiService.planMobilizationExecution
      .deletePlanMobilizationExecution(props.cell.row.original.id)
      .then((response) => {
  ...
  render() {
    return (
      <ActionCell
        ...
        displayEdit={userHasRole(54)}
        displayDelete={userHasRole(57)}
      />
    );
  }
}
...
MobilizationExecutionActionCell.propTypes = {
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(MobilizationExecutionActionCell),
);
