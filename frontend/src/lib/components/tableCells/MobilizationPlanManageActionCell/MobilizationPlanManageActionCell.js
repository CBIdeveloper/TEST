...
class MobilizationPlanManageActionCell extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  openEditPage = () => {
    ...
    const query = createQuery({
      [QueryType.DETAIL_ID]: props.cell.row.original.id,
    });
    props.history.push({
      pathname: Path.editMobilizationPlanPath,
      search: query,
    });
  };
  ...
  deleteFunction = (callback) => {
    ...
    ApiService.planMobilizationPlan
      .deletePlanMobilizationPlan(props.cell.row.original.id)
      .then((response) => {
  ...
  render() {
    return (
      <ActionCell
        ...
        displayEdit={userHasRole(59)}
        displayDelete={userHasRole(62)}
      />
    );
  }
}
...
MobilizationPlanManageActionCell.propTypes = {
...
export default withRouter(
  connect(
    mapStateToProps,
    mapDispatchToProps,
  )(MobilizationPlanManageActionCell),
);
