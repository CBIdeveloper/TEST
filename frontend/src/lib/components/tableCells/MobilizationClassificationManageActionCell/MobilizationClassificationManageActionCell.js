...
class MobilizationClassificationManageActionCell extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  openEditPage = () => {
    ...
    props.history.push({
      pathname: Path.editMobilizationClassificationPath,
      search: query,
    });
  };
  ...
  deleteFunction = (callback) => {
    ...
    ApiService.planMobilizationClassification
      .deletePlanMobilizationClassification(props.cell.row.original.id)
      .then((response) => {
  ...
  render() {
    return (
      <ActionCell
        ...
        displayEdit={userHasRole(49)}
        displayDelete={userHasRole(52)}
      />
    );
  }
}
...
MobilizationClassificationManageActionCell.propTypes = {
...
export default withRouter(
  connect(
    mapStateToProps,
    mapDispatchToProps,
  )(MobilizationClassificationManageActionCell),
);
