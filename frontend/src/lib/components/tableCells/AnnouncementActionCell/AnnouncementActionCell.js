...
class AnnouncementActionCell extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  openEditPage = () => {
    ...
    props.history.push({
      pathname: UrlParser([
        ...
        Path.editSystemAnnouncementPath,
      ]),
      search: query,
    });
  };
  ...
  deleteFunction = (callback) => {
    ...
    ApiService.announcement
      .deleteAnnouncement(props.cell.row.original.id)
      .then((response) => {
  ...
  render() {
    return (
      <ActionCell
        ...
        displayEdit={userHasRole(2)}
        displayDelete={userHasRole(3)}
      />
    );
  }
}
...
AnnouncementActionCell.propTypes = {
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(AnnouncementActionCell),
);
