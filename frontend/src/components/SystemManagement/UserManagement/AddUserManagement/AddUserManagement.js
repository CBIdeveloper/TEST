...
class AddUserManagement extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      isClearText: false,
  ...
  initState = () => {
    ...
    ApiService.mobilizationClassification
      ...
      .then((response) => {
        const data = response.mobilizationClassificationList.map((item) => ({
          text: item.classificationName,
          value: item.id,
          mobilizationPlanId: item.mobilizationPlanId,
        }));
        this.setMobilizationClassificationList(data);
      });
  ...
  onSubmit = async (values) => {
    const { props, state } = this;
    const result = Object.keys(state.checkedState)
      .map((key) => {
    ...
    let request = new SysUserAccountRequest({
      ...values,
      braidingCategoryIdList: values.mobilizationPlanList
        .map((item) => item.braidingCategoryIdList)
        .flat(),
    ...
    if (
      ...
      (values.agencyType == '3' || values.agencyType == '4')
    ) {
      alert('編管類別必填');
    } else {
  ...
  handleMilitarylevelOnChanged = async ({ value, field, setFieldValue }) => {
    const { state } = this;
        ...
        {values.agencyType === '1' ? (
          <UnitInput2
            ...
            firstLevelPlaceholder={
              props.language.register.firstlevelAgencyIdHint
            }
            ...
            secondLevelPlaceholder={
              props.language.register.secondaryAgencyIdHint
            }
          ...
          <UnitInput2
            ...
            secondLevelPlaceholder={
              props.language.register.firstlevelUnitIdHint
            }
          ...
          <TextInput
            ...
            inputPlaceholder={
              props.language.register.maintainManufacturerNameHint
            }
    ...
    if (isPlansponsor) {
      ...
      if (businessPlan === '4') {
        if (agencyType === '4') {
          if (
            levelId != undefined &&
            militaryAgencyId != undefined &&
            secondlevelmilitaryAgency != undefined &&
            this.state.isReportClear
          ) {
            return 34;
          } else if (
            levelId != undefined &&
  ...
  render() {
    ...
    return (
      <Container className="add-user-management">
        <SectionTitle title={props.language.addUserManagement.title} />
        <Formik
          enableReinitialize
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(AddUserManagement),
);
