...
import ImportantPolicyTable from '../../../utils/tables/businessManagement/SubMenuTable/ImportantPolicyTable';
...
import './ImportantPolicy.scss';

class ImportantPolicy extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      title: '',
    ...
    this.table = new ImportantPolicyTable(
      this.setTableData,
      this.setTableCount,
    );
  ...
  render() {
    ...
    return (
      <div className="important-policy">
        <div className="title-container">
          <SectionTitle
            title={props.language.businessManagement.subMenus.importantPolicy}
          />
          <ButtonDiv display={userHasRole(118)}>
            <NavLink
              to={`${props.location.pathname}/${Path.addImportantPolicyPath}`}
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ImportantPolicy),
);
