...
import ExcellenceRecognitionTable from '../../../utils/tables/businessManagement/SubMenuTable/ExcellenceRecognitionTable';
...
import './ExcellenceRecognition.scss';

class ExcellenceRecognition extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      title: '',
    ...
    this.table = new ExcellenceRecognitionTable(
      this.setTableData,
      this.setTableCount,
    );
    this.table.typeId = 9;
  ...
  search = () => {
    ...
    if (state.announcementUser !== '') {
      queryList.push(
        QueryHelper.contains(['announcedUserAccount', 'name'], state.announcementUser, 'string'),
      );
    }
  ...
  render() {
    ...
    return (
      <div className="excellence-recognition">
        <div className="title-container">
          <SectionTitle
            title={
              props.language.businessManagement.subMenus.excellenceRecognition
            }
          />
          <ButtonDiv display={userHasRole(142)}>
            <NavLink
              to={`${props.location.pathname}/${Path.addExcellenceRecognitionPath}`}
...
ExcellenceRecognition.propTypes = {
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ExcellenceRecognition),
);
