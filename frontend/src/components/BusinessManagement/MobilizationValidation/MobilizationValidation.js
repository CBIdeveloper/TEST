...
import MobilizationValidationTable from '../../../utils/tables/businessManagement/SubMenuTable/MobilizationValidationTable';
...
import './MobilizationValidation.scss';

class MobilizationValidation extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      title: '',
    ...
    this.table = new MobilizationValidationTable(
      this.setTableData,
      this.setTableCount,
    );
    this.table.typeId = 7;
  ...
  render() {
    ...
    return (
      <div className="mobilization-validation">
        <div className="title-container">
          <SectionTitle
            title={
              props.language.businessManagement.subMenus.mobilizationValidation
            }
          />
          <ButtonDiv display={userHasRole(136)}>
            <NavLink
              to={`${props.location.pathname}/${Path.addMobilizationValidationPath}`}
...
MobilizationValidation.propTypes = {
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(MobilizationValidation),
);
